...
function showProgress(job, prefix = '') {
  const elapsed = Math.max(0, Math.floor((Date.now() - Date.parse(job.startedAt)) / 1000)) || 0;
  const lines = job.logs || [], step = lines.filter(line => line.trim()).at(-1);
  const completed = lines.filter(line => /^\s{2}/.test(line)).length;
  const label = job.command === 'diagnose' ? `诊断进行中，已完成 ${completed} 项请求` : '操作进行中';
  notice(`${prefix}${label}，已用时 ${elapsed} 秒。${step ? `当前：${step.trim()}。` : ''}${job.command === 'diagnose' ? '逐项等待 API 返回，超时也会计入结果，请勿重复点击。' : ''}`);
}
async function waitForExistingJob() {
  let job = await api('/api/job');
  while (job.state === 'running') {
    lock(true);
    showProgress(job, '已接回后台任务：');
...
async function run(command) {
  ...
  const perform = async action => {
    let accepted;
    try { accepted = await api('/api/run', { command: action, options: selected }); }
    catch (error) {
      if (error.status !== 409 || action !== 'inspect') throw error;
      // A reload or another tab may still own the server's job. Wait for it;
      // retry only the read-only inspect, never diagnosis or configuration writes.
      await waitForExistingJob();
      accepted = await api('/api/run', { command: action, options: selected });
    }
    let job;
    do {
      await new Promise(resolve => setTimeout(resolve, 400));
      ...
      if (sameSelection()) { $('logs').textContent = [...logs, ...(job.logs || [])].join('\n') || '正在读取或处理…'; $('logs').scrollTop = $('logs').scrollHeight; if (job.state === 'running') showProgress(job); }
    } while (job.state === 'running');
    logs.push(...(job.logs || []));
...
resetContext();
lock(true);
api('/api/defaults').then(async config => {
  $('home').value = config.home; $('profile').value = config.profile; $('binary').value = config.binary; $('insecure').checked = config.allowInsecureHttp; selection = selectionKey();
  await waitForExistingJob();
  lock(false);
  return run('inspect');
