(() => {
  ...
  async function openCodexZh() {
    ...
    openDialog("汉化 Codex", `<div class="home-zh-pocket"><div class="feature-icon mint">汉</div><div class="managed-detail"><div class="title-line"><strong>中文界面组件</strong><span class="status-pill ${stateClass}">${stateText}</span></div><p>从当前官方 Codex 构建独立中文副本；官方安装、Key 和会话不会被覆盖。</p><small>${h(detail)}</small></div></div><div class="managed-actions home-zh-actions">${button("install-zh", zh.installed ? (zh.compatible ? "重新安装中文版" : "更新中文版") : "安装中文版", !result.official.installed || !zh.componentValid ? "disabled" : "")}${button("launch-zh", "启动中文版", !zh.installed || !zh.compatible ? "disabled" : "")}${button("remove-zh", "卸载中文版", !zh.installed ? "disabled" : "")}${button("recoveries-zh", "恢复记录")}</div><div class="modal-note">${zh.installed && zh.compatible ? "已是当前版本，直接点击“启动中文版”。" : zh.installed ? "官方版本已变化，请先更新中文版。" : "先安装中文版，再点击启动。"}</div>`, null);
    dialog.querySelector('[data-action="' + (zh.installed && zh.compatible ? 'launch-zh' : 'install-zh') + '"]').classList.replace('ghost','primary');
