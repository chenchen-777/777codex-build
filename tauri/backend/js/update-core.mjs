...
export function validateUpdateManifest(value, { product, channel, publicKey }) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new AppError("更新清单格式无效", "UPDATE_MANIFEST_INVALID", 502);
  if (value.schemaVersion !== 1 || value.product !== product || value.channel !== channel) throw new AppError("更新清单不属于当前客户端或通道", "UPDATE_MANIFEST_MISMATCH", 409);
  ...
  if (!pkg || pkg.kind !== "app-patch-zip") throw new AppError("当前客户端不支持此更新包类型", "UPDATE_PACKAGE_UNSUPPORTED", 409);
  ...
  return { ...value, package: { ...pkg, sha256: pkg.sha256.toLowerCase(), url: url.href } };
}
