...
for(const name of ['server.mjs','feature-api.mjs','windows-api.mjs','tauri-update-windows.ps1','tauri-update-macos.mjs'])await copy(join(source,'backend','scripts',name),join(backend,'scripts',name));
...
await writeFile(join(backend,'js','build-info.mjs'),`export const BUILD_INFO=Object.freeze(${JSON.stringify({product:'777codex-tauri',version:RELEASE.version,revision:RELEASE.revision,revisionLabel:RELEASE.label,channel:RELEASE.channel,updateManifestUrl:updateManifestUrl('darwin',process.arch)})});\n`);
