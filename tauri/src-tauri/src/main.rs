...
#[tauri::command]
fn window_action(window:tauri::WebviewWindow,state:State<'_,Backend>,action:String)->Result<Value,String>{
    ...
    match action.as_str(){
        "smoke-pass"=>{
            ...
            std::fs::write(path,json!({"ok":true,"framework":"tauri","uiLoaded":true,"platform":std::env::consts::OS,"arch":std::env::consts::ARCH}).to_string()).map_err(|_|"无法保存验收结果")?;
...
fn main(){
    ...
    let result=tauri::Builder::default().invoke_handler(tauri::generate_handler![window_action]).setup(|app|{
        ...
        let window_data=if cfg!(target_os="macos"){
            let base=if isolated{std::env::var_os("MANAGER777_STATE_ROOT").map(std::path::PathBuf::from).unwrap_or(app.path().app_cache_dir()?.join("isolated"))}else{app.path().app_data_dir()?};
            base.join("Window")
