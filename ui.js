(() => {
  ...
  function openPage(pageName) {
    ...
    const navigation = ['enhance', 'sessions', 'extensions', 'codex'].includes(pageName) ? 'tools' : pageName === 'logs' ? 'settings' : pageName;
  ...
  function renderHomeModel() {
    ...
    $('#home-model-message').textContent = !profile ? '请先添加或同步一个文字 Key。' : models ? (select.value ? `列表支持此模型；实际对话需测试。${homeDraft.dirty ? '启动时应用本次选择。' : ''}` : '当前 Key 不支持原模型，请重新选择。') : '启动前检查 Key 是否支持此模型。';
  }
  ...
  $('#home-sync-models').addEventListener('click', async event => {
    ...
    renderHomeModel();
    if (!result) $('#home-model-message').textContent = '同步失败，暂不能确认模型支持。请重试。';
  });
